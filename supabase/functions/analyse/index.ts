...
For FLATLAY mode, identify each distinct clothing item laid out separately in the photo, even if items overlap or touch.
{
  "mode": "flatlay",
  "items": [
    {
...
FLATLAY RULES:
- box coordinates are normalised 0 to 1, relative to the full image width and height
- x and y are the top-left corner of a tight bounding box around the item
- If items overlap, draw the box around the visible extent of that item only
- Identify every distinct item, do not merge separate items into one box
- Ignore hangers, backgrounds, floor, bed, or body parts`;
...
function buildCheckItemMessages(imageBase64: string, season: string, mode: string, swatchLabel?: string) {
  const modeInstructions: Record<string, string> = {
    ...
    flatlay: "This photo shows multiple separate clothing items laid out or photographed together, not worn as an outfit. Identify each distinct item individually with a tight bounding box, and check each one against my season.",
  };
